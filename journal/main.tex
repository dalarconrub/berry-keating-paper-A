...
for all $\alpha > 0$, then RH is true. The Conrey--Snaith triple
...
has the correct modulus but unknown phase; with phase $= \mathrm{sgn}(\mathrm{sinc})$,
the Fredholm determinant gives the wrong sign for
$\delta\sigma$~\cite{alarcon2026b}. A Born approximation captures only
$26\%$ of $|\delta\sigma|$; the full Conrey-Snaith decomposition~\cite{alarcon2026b}
shows that the $R_3$ channel contributes $c_{R_3} = -1.25$ (opposite sign),
while the gap probability channel contributes $c_E = +2.50$, with
