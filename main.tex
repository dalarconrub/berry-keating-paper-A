...
\date{March 2026\\[2pt]
\small \textsc{doi}: \href{https://doi.org/10.5281/zenodo.19268721}{10.5281/zenodo.19268721}}
...
\bigskip
\noindent\textbf{Keywords:} Riemann zeta function, gap ratio statistics, random matrix theory, GUE, Berry--Keating conjecture, spacing distribution, number variance, spectral rigidity, Platt zeros, convergence rate
...
As a further robustness check, we downloaded four additional Platt files
from the LMFDB at $\log T \in \{23.3, 23.5, 23.7, 24.0\}$, filling the
gap between 23.1 and 24.1. Including these (25~points total), the fit gives
$c = 1.251 \pm 0.039$, $R_\infty = 0.59888 \pm 0.00012$, $\chi^2/\mathrm{dof} = 0.44$
(23~dof)---consistent with the 21-point result within $0.2\sigma$ in $c$.
The extended 25-point dataset is provided as \texttt{dataset\_v7\_25pts.dat}
...
\emph{normalised} form factor proportional to $T^{2\sigma_0 - 2}/(\log T)^{3}$,
which decays polynomially for $\sigma_0 < 1$ but at a rate slower than
the Berry-Keating correction $1/\log^2 T$ when $\sigma_0$ is close to $1$.
...
\subsection{Validation at high $\log T$ with sparse Odlyzko zeros}

As an independent consistency check, we measure $\langle r\rangle$ from
three sparse Odlyzko datasets at much higher heights than the main
dataset~\cite{odlyzko1987}. These contain only ${\sim}\,10{,}000$ zeros each
(giving $\sigma_r \approx 0.0023$, roughly $10\times$ larger than the Platt
points), but probe $\log T$ values far beyond the Platt range:

\begin{table}[ht]
\centering
\caption{Consistency check with sparse Odlyzko zeros (not included in
the Model~A fit). Predictions use the best-fit parameters
from~\eqref{eq:modelA}.}
\label{tab:sparse}
\begin{tabular}{lccccr}
\toprule
Dataset & $\log T$ & $N$ & $\langle r\rangle$ & $\sigma_r$ & Residual \\
\midrule
zeros3 & 26.3 & 10{,}000 & 0.60004 & 0.00232 & $-0.29\sigma$ \\
zeros4 & 46.4 & 10{,}000 & 0.59716 & 0.00231 & $-1.01\sigma$ \\
zeros5 & 48.7 & 10{,}000 & 0.60193 & 0.00229 & $+1.09\sigma$ \\
\bottomrule
\end{tabular}
\end{table}

All three points are consistent with Model~A within $\sim\!1\sigma$.
At $\log T \approx 47$--$49$, the correction $c/\log^2 T \approx 0.0005$
is negligible compared to $\sigma_r$, so these points effectively measure
$R_\infty$ directly. Their mean, $\langle r\rangle = 0.5997 \pm 0.0013$,
is consistent with $R_\infty = 0.59891 \pm 0.00013$. This extends the
validation of Model~A from a factor $2.5\times$ in $\log T$ (the main
dataset: $9.7$--$24.1$) to a factor $5\times$ ($9.7$--$48.7$).
...
is smooth (the exact kernel
$K^{\mathrm{BK}} = \sqrt{\mathrm{sinc}^2 + \varepsilon\,\delta Y_2}$
regularises the apparent singularity).
The Conrey--Snaith triple correlation formula~\cite{alarcon2026b}
decomposes $c$ into two channels: the $R_3$ channel
($c_{R_3} = -1.6$, computed \emph{ab initio} from the triple correlation)
and the gap probability channel ($c_E = +2.8$, obtained by difference),
whose sum $c_{R_3} + c_E = c_{\mathrm{emp}}$ reproduces the empirical
value exactly. Nevertheless, computing $c$ analytically in closed form
from $\delta b_2$ remains an open problem.

\subsection{Companion papers}

A companion paper~\cite{alarcon2026d} proves that every $n$-level
correlation functional of Riemann-zero ordinates factors through the
projection $(\sigma_j,\gamma_j)\mapsto\{\gamma_j\}$, so that no
spacing-based statistic can detect off-line zeros; it further classifies
three families of structural obstructions encountered in the derivation
program for $c$. The Conrey--Snaith triple correlation formula
decomposes $c$ into $c_{R_3} = -1.6$ and
$c_E = +2.8$, with $c_{R_3} + c_E = c_{\mathrm{emp}}$~\cite{alarcon2026b}.
...
\url{https://github.com/dalarconrub/berry-keating-paper-A}.
...
jimbo1981}; for the joint spacing density
...
the kernel phase is unknown, and perturbative expansions capture at most
$26\%$ of $|\delta\sigma|$. The full analysis of this obstruction and the
two-channel decomposition of $c$ are presented in~\cite{alarcon2026b}.
...
analytically in closed form from $\delta b_2$ remains an open problem.
