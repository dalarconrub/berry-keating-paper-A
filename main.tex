...
\medskip
\noindent\textbf{MSC 2020:} 11M26 (primary), 60B20, 15B52, 11N05
\end{abstract}
...
is smooth~\cite{alarcon2026c}. The Conrey--Snaith triple correlation formula~\cite{alarcon2026b} decomposes $c$ into two channels: the $R_3$ channel ($c_{R_3} = -1.25$, computed \emph{ab initio} from the triple correlation) and the gap probability channel ($c_E = +2.50$, obtained by difference), whose sum $c_{R_3} + c_E = c_{\mathrm{emp}}$ reproduces the empirical value exactly. Nevertheless, computing $c$ analytically in closed form from $\delta b_2$ remains an open problem.
...
correlation formula decomposes $c$ into $c_{R_3} = -1.25$ and
$c_E = +2.50$, with $c_{R_3} + c_E = c_{\mathrm{emp}}$~\cite{alarcon2026b}.
